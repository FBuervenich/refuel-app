...
const RefuelingsController: RestController = {
  ...
  index: inject(
    ({ getAllRefuelings, refuelingSerializer }) =>
      (req, res, next) => {
  ...
  create: inject(
    ({ createRefueling, refuelingSerializer }) =>
      (req, res, next) => {
  ...
  update: inject(
    ({ updateRefueling, refuelingSerializer }) =>
      (req, res, next) => {
        const { SUCCESS, ERROR, VALIDATION_ERROR, NOT_FOUND } =
